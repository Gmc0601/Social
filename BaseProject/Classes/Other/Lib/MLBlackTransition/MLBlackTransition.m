...
@end
@implementation UIViewController(__MLBlackTransition)
...
- (void)handleNavigationTransition:(UIPanGestureRecognizer*)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        //		if (point.x >100) {
			     UIViewController *controller =  [recognizer.__MLBlackTransition_NavController popViewControllerAnimated:YES];
        if (controller && [controller respondsToSelector:@selector(beginPopViewController)]) {
            [controller beginPopViewController];
        }
        //		}
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)recognizer
{
    ...
    UIView *rootView = navVC.visibleViewController.view;// Todo :change by yangyanan 11.10
    
    
    
    if (rootView.disableMLBlackTransition){ //这个view忽略了拖返
        return NO;
    }
    ...
    UIViewController *visibleController = navVC.visibleViewController;
    if ([visibleController respondsToSelector:@selector(beginPopViewController)]) {
        //		CGPoint translation = [recognizer locationInView:navVC.view];
        //		NSLog(@"x   ==== %f",translation.x);
        //		if (translation.x >100) {
        [visibleController beginPopViewController];
        //		}
