...
@interface EaseMessageViewController ()<EaseMessageCellDelegate,EaseMessageViewControllerDelegate, EaseMessageViewControllerDataSource>
{
...
- (void)viewDidLoad {
    ...
    //返回的手势
    UISwipeGestureRecognizer *gesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(backAction)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        RedPagCellTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
         ...
         */
         ...
         [ConfigModel saveString:self.chattername forKey:@"chatNick"];
        if (model.isSender) {
            model.nickname = [ConfigModel getStringforKey:User_Nick];
            model.avatarURLPath = [ConfigModel getStringforKey:User_url];
        }else {
            model.nickname = self.chattername;
            model.avatarURLPath = self.chatterhead;
        }
        
        
        if ([model.text isEqualToString:@"[红包]"] && !IsNULL(model.message.ext) ) {
            model.cellHeight =  90;
        }
        
        if ([model.text isEqualToString:@"红包已被领取"]) {
            model.cellHeight =  15;
            cell = [[RedPagCellTableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:CellIdentifier model:model];
            [cell setupsubview:model];
            return cell;
...
//            if (sendCell == nil) {
        ...
        sendCell.redpagBlock = ^(BOOL sender) {
             NSDictionary *dic =  model.message.ext;
            NSString *str = dic[@"redbagId"];
            
            NSDictionary *params = @{
                                  @"id" : str
                                  };
            
            [HttpRequest postPath:@"_red_particulars_001" params:params resultBlock:^(id responseObject, NSError *error) {
                ...
                if ([datadic[@"error"] intValue] == 0) {
                    ...
                    RedPagClass *model = [[RedPagClass alloc] init];
                    model.nick = info[@"nickname"];
                    model.head = info[@"avatar_url"];
                    model.id = [NSString stringWithFormat:@"%@", info[@"id"]];
                    model.count = [NSString stringWithFormat:@"%@", info[@"red_packet"]];
                    if ([info[@"has_bring"] intValue] == 1) {
                        model.get = YES;
                    }else {
                        model.get = NO;
                    }
                    ...
                    if (sender) {
                        ...
                        RedPagDetialViewController *vc = [[RedPagDetialViewController alloc] init];
                        vc.model = model;
                        [self.navigationController pushViewController:vc animated:YES];
                        
                    }else {

                        RedPagAlter *alter = [[RedPagAlter alloc] initWithFrame:FRAME(0, 0, kScreenW, kScreenH) withModel:model];
                        alter.openBlock = ^{
                            
                            NSDictionary *dic = @{
                                                  @"has_open" : @(1)
                                                  };
                            [self sendTextMessage:@"红包已被领取" withExt:dic];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    id object = [self.dataArray objectAtIndex:indexPath.row];
    if ([object isKindOfClass:[NSString class]]) {
        NSString *titleStr = object;
//        if ([titleStr isEqualToString:@"[红包]"]) {
//            return 90;
//        }else {
...
//        }
        ...
        id<IMessageModel> model = object;
        if ([model.text isEqualToString:@"[红包]"]) {
            model.cellHeight = 90;
        }
        if ([model.text isEqualToString:@"红包已被领取"]) {
            model.cellHeight =  15;
...
- (void)moreViewLocationAction:(EaseChatBarMoreView *)moreView
{
    ...
    AddRedPackViewController *red = [[AddRedPackViewController alloc] init];
    red.mobile = self.mobile;
    WeakSelf
    red.moneybackBlock = ^(NSString *money) {
    //   发消息
//        - (void)sendTextMessage:(NSString *)text withExt:(NSDictionary*)ext
        NSDictionary *dic = @{
                              @"redbagId" : money
                              };
        [weakSelf sendTextMessage:@"[红包]" withExt:dic];
    };
