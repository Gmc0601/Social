...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    UIImageView *logoimageView; UIButton *btn;
    if ([model.text isEqualToString:@"[红包]"]) {
        if (model.isSender) {
            logoimageView = [[UIImageView alloc] initWithFrame:FRAME(kScreenW - 130, 0, 70, 90)];
            btn = [[UIButton alloc] initWithFrame:FRAME(kScreenW - 130, 0, 70, 90)];
            logoimageView.image = [UIImage imageNamed:@"bg_wk"];
        }else {
            logoimageView = [[UIImageView alloc] initWithFrame:FRAME(60, 0, 70, 90)];
            btn = [[UIButton alloc] initWithFrame:FRAME(60, 0, 70, 90)];
            logoimageView.image = [UIImage imageNamed:@"bg_k"];
        }
...
- (void)click {
    if (self.redpagBlock) {
        self.redpagBlock(self.model.isSender);
    }
}
